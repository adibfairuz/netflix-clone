...
const ModalMovieCard: React.FC<ModalMovieCardProps> = ({
    ...
    } = useQuery(`movieCredits-${data.id}`, () => getMovieCredits(data.id), { enabled: false });
                            ...
                            !isLoadingMovieCredits && cast.length ? (
                                <>
                                    {cast.filter((item, i) => i < 5).map((item) => (
                                        <span className={style.value} key={item.cast_id}>
                                            {item.name}
                                            ,
                                            {' '}
                                        </span>
